...
function getPaidFee(fee: any) {
  if (fee.details?.paidFee) return fee.details.paidFee
  return fee.amount
}
...
export function FeeDetailsScreen({ onBack }: FeeDetailsScreenProps) {
  ...
  return (
    <div className="h-full w-full flex flex-col bg-white">
      {/* Header */}
      <div className="bg-[#473F97] pt-12 pb-5 px-4 relative overflow-hidden">
      ...
      <div className="flex bg-white border-b border-[gray-100]">
        {feeTabs.map((tab) => (
          <button
            ...
            className={`flex-1 py-3 px-1 text-xs font-medium transition-colors whitespace-nowrap ${
              activeTab === tab
                ? "text-[#FD3667] border-b-2 border-[#FD3667]"
                : "text-[#473F97]"
            }`}
      ...
      <div className="flex-1 overflow-auto px-3 py-3 flex flex-col gap-2">
        {currentFeeData.length > 0 ? (
          currentFeeData.map((fee, index) => {
            const isOpen = expandedId === fee.id
            const details = fee.details ?? defaultDetails
            const paidFee = getPaidFee(fee)
            
            // Show all items when nothing is expanded
            // When an item is expanded, limit the visible items below it
            let shouldShow = true
            if (expandedId !== null && activeTab === "School Fee") {
              const expandedIndex = currentFeeData.findIndex(f => f.id === expandedId)
              // January (id 1) or December (id 2) expanded: show until September
              if (expandedId === 1 || expandedId === 2) {
                // Find September's index and show until then
                const septemberIndex = currentFeeData.findIndex(f => f.month === "September")
                shouldShow = index <= septemberIndex
              } else {
                // For other months, show expanded item + 2 items below
                shouldShow = index <= expandedIndex + 2
              }
            }
            ...
            return (
              <div key={fee.id} className="rounded-2xl overflow-hidden" style={{ backgroundColor: "#D4F5FF" }}>
                {/* Fee Row */}
                <button
                  onClick={() => setExpandedId(isOpen ? null : fee.id)}
                  className="w-full px-4 py-4 flex flex-col"
                >
                  <div className="flex items-center justify-between">
                    <div className="text-left">
                      <p className="text-sm" style={{ color: "#707070" }}>
                        {activeTab} for {fee.month}
                      </p>
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        {fee.paid && (
                          <span className="px-3 py-0.5 bg-[#12B264] text-white text-xs rounded-full font-medium">
                            Paid
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex flex-col items-end gap-1">
                      <span className="text-sm text-gray-400">{fee.date}</span>
                      {activeTab === "School Fee" && (
                        <>
                          {isOpen
                            ? <ChevronUp className="w-5 h-5 text-[#FD3667]" />
                            : <ChevronDown className="w-5 h-5 text-[#FD3667]" />
                          }
                        </>
                      )}
                    </div>
                ...
                {/* Expanded Details - Only for School Fee */}
                {isOpen && activeTab === "School Fee" && (
                  <div className="px-4 py-4 w-full" style={{ backgroundColor: "#B8EDFD" }}>
                    <div className="flex flex-col gap-3 w-full">
                      <div className="flex justify-between items-center text-sm gap-2 w-full">
                        <span className="text-gray-600">Total Fee</span>
                        <span className="text-black text-right">₹ {details.totalFee ? details.totalFee.toLocaleString("en-IN") : fee.amount.toLocaleString("en-IN")}</span>
                      </div>
                      <div className="flex justify-between items-center text-sm gap-2 w-full">
                        <span className="text-gray-600">Extra Fee</span>
                        <span className="text-gray-600 text-right">₹ {details.extraFee ? details.extraFee.toLocaleString("en-IN") : "0"}</span>
                      </div>
                      <div className="flex justify-between items-center text-sm gap-2 w-full">
                        <span className="text-gray-600">Late Charges</span>
                        <span className="text-gray-600 text-right">₹ {details.lateCharges ? details.lateCharges : "0"}</span>
                      </div>
                      <div className="flex justify-between items-center text-sm gap-2 w-full">
                        <span className="text-gray-600">Discount ({details.discountPercent})</span>
                        <span className="text-gray-600 text-right">- ₹ {details.discount}</span>
                      </div>
                      <div className="flex justify-between items-center text-sm font-bold pt-3 border-t border-[#89D8F0] gap-2 w-full">
                        <span className="text-gray-800">Paid Fee</span>
                        <span className="text-black text-right">₹ {paidFee.toLocaleString("en-IN")}</span>
                      </div>
                    </div>
                  </div>
                )}
